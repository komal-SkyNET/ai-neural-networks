...
agent = DQNAgent()
if len(sys.argv)>1:
    if sys.argv[1] == '--load-model-file':
        fn = sys.argv[2]
        print('loading model from file...{}'.format(fn))
        agent.model = keras.models.load_model('models-breakout/{}'.format(fn))

print(agent.model.summary())

def save(sig, fr):
    print('saving model & exiting')
    agent.model.save('models-breakout/breakout-{}.model'.format(int(time.time())))
    exit(0)
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        if min_reward > MIN_REWARD:
            ...
            agent.model.save('models-breakout/{}__{}max_{}avg_{}min__{}.model'.format(MODEL_NAME,
